...
export default function LoginPage() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      const res = await authGoogle(idToken, "candidate");
      login(res.token, res.user);
      router.push("/");
    } catch (err: unknown) {
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm shadow-teal-500/5">
          ...
          <button
            ...
            disabled={loading}
            className="w-full flex items-center justify-center gap-3 px-4 py-2.5 rounded-lg border border-gray-200 text-sm font-medium text-foreground hover:bg-teal-50 transition-colors disabled:opacity-50"
          >
