...
def search_locations(query: str, limit: int = 20) -> list[Location]:
    """Return municipalities/cities matching *query*.

    Searches both real municipalities and virtual city aliases.
    Uses accent-insensitive substring matching with prefix results first.
    """
    q = query.lower().strip()
    if not q:
        return []
