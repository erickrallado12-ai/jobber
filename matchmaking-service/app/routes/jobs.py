...
async def _get_current_recruiter_id(
    user=None,
    db: AsyncSession = Depends(get_db),
) -> uuid.UUID:
    from app.routes.auth import get_current_user as _get_user
    raise HTTPException(401, "Use get_current_user dependency")
...
async def create_job(
    ...
    text = _build_embedding_text("job", payload.model_dump())
...
async def list_jobs(
    ...
    limit: int = 50,
    db: AsyncSession = Depends(get_db),
    user=Depends(__import__("app.routes.auth", fromlist=["get_optional_user"]).get_optional_user),
):
...
async def update_job(
    ...
    if update_data:
        ...
        if any(f in update_data for f in emb_fields):
            ...
            text = _build_embedding_text("job", emb_data)
            emb = await generate_embedding(text)
            ...
            if existing_emb:
                ...
                existing_emb.content_hash = _content_hash(text)
            else:
                db.add(
                    Embedding(
                        entity_type=EmbeddingEntity.job,
